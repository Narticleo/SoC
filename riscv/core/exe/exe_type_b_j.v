...
module exe_type_b_j(
    ...
    output reg branch_predict_fail_o,
    ...
    always @(*) begin
        ...
        `INST_TYPE_B: begin
            ...
            `INST_BNE:begin
                ...
                jump_enable_o = 1'b0;
                branch_predict_fail_o = ~(~op1_eq_op2);//跳轉條件不成立 預測錯誤
                if(branch_predict_fail_o) begin
                    jump_addr_o = pc + 8;
                end
            end
            `INST_BEQ:begin
                //jump_addr_o = pc + b_imm;
                jump_enable_o = 1'b0;
                branch_predict_fail_o = ~(op1_eq_op2);
                if(branch_predict_fail_o) begin
                    jump_addr_o = pc + 8;
                end
            end
            `INST_BLT:begin
               // jump_addr_o = pc + b_imm;
               jump_enable_o = 1'b0;
                branch_predict_fail_o = ~(~op1_ge_op2_signed);
                if(branch_predict_fail_o) begin
                    jump_addr_o = pc + 8;
                end
            end
            ...
            `INST_BLTU:begin
                //jump_addr_o = pc + b_imm;
                jump_enable_o = 1'b0;
                branch_predict_fail_o = ~(~op1_ge_op2_unsigned);
                if(branch_predict_fail_o) begin
                    jump_addr_o = pc + 8;
                end
            end
